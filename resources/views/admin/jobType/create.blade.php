        ...
        <table class="table">
            ...
            @forelse($jobTypes as $key=>$jobType)
                <tr id="dept-{{ $jobType->id }}">
                    ...
                    <td>
                        <a href="javascript:;" data-dept-id="{{ $jobType->id }}" class="btn btn-sm btn-danger btn-rounded delete-department"><i class="fa fa-times"></i> </a>
                        <a href="javascript:;" data-dept-id="{{ $jobType->id }}" class="btn btn-sm btn-info btn-rounded edit-department"><i class="fa fa-pencil"></i> </a>
                    </td>
                </tr>
            @empty
                <tr>
...
<script>
            ...
            success: function (response) {
                var options = [];
                    ...
                    $.each(rData, function( index, value ) {
                        var selectData = '';
                        selectData = '<option value="'+value.id+'">'+value.job_type+'</option>';
    ...
    $('#update-department').click(function () {
        ...
        $.easyAjax({
            ...
            container: '#createdepartment',
            type: "POST",
            data: {'_token':token, '_method':'PUT','job_type':name  },
            success: function (response) {
    ...
    $('body').on('click', '.delete-department', function(){
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('admin.job-type.destroy',':id') }}";
                url = url.replace(':id', id);
